...
async function screenshot(page, name: string) {
  await page.screenshot({ path: `../../docs/screenshots/${name}`, fullPage: true });
}

test.describe("Editor flows with screenshots", () => {
  test("expand suite, open editor, run test, save, and batch run", async ({ page }) => {
    ...
    const suiteItem = page.locator('#suites li', { hasText: 'example.yaml' }).first();
    ...
    if (await method.count()) await method.fill('GET');
    ...
    const closeBtn = modal.locator('[data-action="close"], .modal-close, button[title="Close"], #ed_close');
