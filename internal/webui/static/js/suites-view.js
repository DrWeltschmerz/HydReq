...
(function(){
    ...
    try{
      ...
      if ((!list || list.length===0) && typeof window !== 'undefined'){
        if (!document.getElementById('hydreq-debug-banner')){
          const banner = document.createElement('div'); banner.id='hydreq-debug-banner'; banner.className='dbg-banner'; banner.textContent = 'HYDREQ-DBG: No suites returned from /api/editor/suites';
  ...
  const titleSpan = document.createElement('span'); titleSpan.className = 'spec-title fs-16'; titleSpan.style.fontWeight='600';
  ...
  const testsDiv = document.createElement('div'); testsDiv.className = 'suite-tests mt-6 p-6 border-l-soft'; testsDiv.style.display='none'; name.appendChild(testsDiv);
