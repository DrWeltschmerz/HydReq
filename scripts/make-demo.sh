...
docker run --rm -v "$PWD:/work" jrottenberg/ffmpeg:4.4-alpine \
  -y -i "/work/$INPUT_WEBM" -vf "fps=24,scale=1400:-1:flags=lanczos,palettegen" \
  "/work/$PALETTE"

docker run --rm -v "$PWD:/work" jrottenberg/ffmpeg:4.4-alpine \
  -y -i "/work/$INPUT_WEBM" -i "/work/$PALETTE" \
  -filter_complex "fps=24,scale=1400:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=sierra2_4a" \
  "/work/$OUTPUT_GIF"
